package com.example.northjosh.services;
...
import com.example.northjosh.models.Vendor;
import com.example.northjosh.models.Product;
import com.example.northjosh.models.Subcategory;
import com.example.northjosh.models.Transaction;
...
public class DBHandler {
	...
	public Connection connectDB() {
		...
		try{
			...
			conn = DriverManager.getConnection("jdbc:postgresql://localhost:5432/raphacare", "postgres", "northjosh");
...
//			DBConnection.disconnect(conn);
	...
	public Product getProductByUpcAndName(String upc, String productName) {
		...
		try {
			...
			while (results2.next()) {
				foundVendor = new Vendor(results2.getInt(1), results2.getString(2));
			}

			if (foundVendor != null) {
				System.out.println(foundVendor.getVendorId());
				foundProduct.setManufacturerInt(foundVendor.getVendorId());
			}
...
//			DBConnection.disconnect(conn);
	 ...
	 */
	public ArrayList<Vendor> getAllManufacturers() {
		...
		try {
			...
			while (results.next()) {
				currVendor = new Vendor(results.getInt(1), results.getString(2));
				vendors.add(currVendor);
			}
//			DBConnection.disconnect(conn);
...
//			DBConnection.disconnect(conn);
	...
	public ArrayList<Vendor> findManufacturersWithName(String nameInput) {
		ArrayList<Vendor> foundVendors = new ArrayList<>();
		...
		try {
			...
			// iterate through ResultSet, adding each manufacturer to list
			while (results.next()) {
				currVendor = new Vendor(results.getInt(1),
						results.getString(2)
				);
				foundVendors.add(currVendor);
			}
//			DBConnection.disconnect(conn);
