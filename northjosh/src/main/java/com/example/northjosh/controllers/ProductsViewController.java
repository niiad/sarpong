package com.example.northjosh.controllers;
...
import com.example.northjosh.models.User;
import com.example.northjosh.services.Authorizer;
import com.example.northjosh.services.DBHandler;
import com.example.northjosh.services.ParseNumbers;
    ...
    public void addProduct() {
        try {
            ...
            if (allowed) {
                ...
                ArrayList<Vendor> vendorList = handler.findManufacturersWithName(manufacturer);
...
//                int subcategoryInt = assignSubcategoryInt(category);
                ...
                Product newProduct = new Product(upc,
                        ...
                        quantity,
                        price,
                        vendorList.get(0).getVendorId(),
                        subcategoryList.get(0).getSubcategoryId(),
                        vendorList.get(0).getVendorName(),
    ...
    public int assignManufacturerInt(String userInput) {
        ...
        ArrayList<Vendor> vendors = handler.getAllManufacturers();

        for (int i = 0; i < vendors.size(); i++) {
            Vendor currVendor = vendors.get(i);
            if (currVendor.getVendorName().equals(userInput)) {
                manufacturerId = currVendor.getVendorId();
            }
        }
